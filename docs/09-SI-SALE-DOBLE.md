...
## 🧹 Que se puede hacer mas rapido: el LIMPIADOR (v41)
...
**Y desde la v41 el cartel rojo ya no sale cuando el juego en realidad si
...
## 🧾 El juego te dice DONDE esta la copia (v41)
...
```
[SpiceEmpire] *** HAY OTRA ClientUI CORRIENDO (v41) *** esta copia (v41) se apaga sola...
```
...
## 🆕 v37/v41: "no cambio nada" aunque pegues todo
...
* La v41 la **borra sola** al arrancar (avisa en el Output con la marca `BASURA`).

### 2. La ClientUI que no corre

Un `Script` **normal** puesto en `StarterPlayerScripts` **no corre**: no dibuja nada y
no avisa. Igual con `Enabled = false`. La v41 lo revisa y lo grita en el Output:
...
* **Placa verde** arriba al centro (cliente): `RONDA v41 ...`.
* **Letrero** flotando arriba del spawn (servidor): `SERVIDOR v41`.
* **Letrerito chiquito** `v41` (se queda siempre, arriba al centro).
* **INVENTARIO** en el Output con la ronda de **cada** archivo.


---

## 🆕 v41: el tablero viejo que "no se iba" aunque pegaras todo
...
**Arreglado en v41**: el tablero se reconoce **por lo que dice**:
...
## 🆕 v41: "Encontre carpetas Remotes de mas" (la leyenda azul)
...
```
'Remotes' (sello v41, 11 remotes)   <- la buena, la que crea el servidor de esta ronda
'Remotes' (SIN sello = vieja, 9 remotes)  <- de una version vieja (tenia 9 remotes)
```

**No es un error y no rompe nada**: el juego usa siempre la que trae sello. La v41 hace
...
* **A mano**: Explorer > `ReplicatedStorage` > clic derecho en **cada** carpeta
  ...
  deja **UN** `Main`: el que al abrirlo diga `RONDA: v41`.
