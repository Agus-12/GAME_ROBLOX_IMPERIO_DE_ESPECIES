...
> de la **v35** (que pide 11) → faltaban **`Shoot`** (del arma, v21) y
...
| `IncomingCall` | v35 | el `Main.luau` es de la v27 o antes |

Y desde la **v35** el cartel ya te lo dice él solito, sin tabla:

```
ARCHIVOS VIEJOS EN STUDIO  --  esta ronda es v35
 - Main.luau del servidor dice NADA (es viejo, ni siquiera dice version); esta ronda es v35
 - GameConfig dice 'v20'; esta ronda es v35
...
O sea: en la **v35** el servidor **estampa su versión** en la carpeta `Remotes`, y el
...
- Si existe y adentro hay **11 cosas** (10 RemoteEvent + 1 RemoteFunction `Action`) → estás en v35. ✅
- Si hay menos → falta pegar el `Main.luau` de esta ronda.

Y esta es la lista completa de la v35 (los 11):
