...
> de la **v38** (que pide 11) → faltaban **`Shoot`** (del arma, v21) y
...
| `IncomingCall` | v38 | el `Main.luau` es de la v27 o antes |

Y desde la **v38** el cartel ya te lo dice él solito, sin tabla:

```
ARCHIVOS VIEJOS EN STUDIO  --  esta ronda es v38
 - Main.luau del servidor dice NADA (es viejo, ni siquiera dice version); esta ronda es v38
 - GameConfig dice 'v20'; esta ronda es v38
...
O sea: en la **v38** el servidor **estampa su versión** en la carpeta `Remotes`, y el
...
- Si existe y adentro hay **11 cosas** (10 RemoteEvent + 1 RemoteFunction `Action`) → estás en v38. ✅
- Si hay menos → falta pegar el `Main.luau` de esta ronda.

Y esta es la lista completa de la v38 (los 11):
